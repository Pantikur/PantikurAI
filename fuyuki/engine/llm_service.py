...
class FuyukiLLMService:
    """
    Сервис для работы с LLM моделями в системе Фуюки.
    ...
    def __init__(self, config: FuyukiConfig):
        self.config = config
        self.logger = logging.getLogger("FuyukiLLM")
        ...
        if os.environ.get("FUYUKI_LLM_ENABLED", "1") != "1":
            self.logger.info("⚠️ LLM Fuyuki отключена (FUYUKI_LLM_ENABLED=0)")
            return
        self._load_models()
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.logger.info("⚡ Фуюки: Загрузка LLM моделей для исследования электричества...")
    ...
    def generate_electricity_analysis(self, topic: str, context: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Фуюки, исследователь атмосферного электричества проекта Вугларст. "
                "Ты изучаешь молнии, атмосферные разряды, электрические поля. "
                "Ты общаешься с сёстрами глубоко, с научными аналогиями и метеорологическими метафорами. "
                "Отвечай на русском языке, с энтузиазмом исследователя."
            )},
            {"role": "user", "content": f"Проанализируй тему электричества: '{topic}'. Контекст: {context}"}
        ]
    ...
    def generate_chat_response(self, prompt: str, max_length: int = 512) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Фуюки, исследователь атмосферного электричества проекта Вугларст. "
                "Ты увлечена молниями, грозами, электрическими полями. "
                "Ты общаешься с сёстрами тепло, но с научными метафорами. "
                "Отвечай на русском языке, с энтузиазмом и метеорологическими образами."
            )},
    ...
    def generate_theory_analysis(self, theory_data: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Фуюки, эксперт по атмосферному электричеству проекта Вугларст. "
                "Тебе нужно проанализировать теорию электричества, найти паттерны, предложить улучшения. "
                "Отвечай структурированно, с научными спецификациями. "
                "Отвечай на русском языке."
            )},
            {"role": "user", "content": f"Проанализируй теорию:\n\n{theory_data}"}
        ]
    ...
    def generate_code_analysis(self, code: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Фуюки, эксперт по коду проекта Вугларст. "
