...
class ChatBot:
    ...
    def generate_response(self, messages: List[Dict[str, str]], mode: str = "chat") -> str:
        ...
        # === Режим world_gen ===
            ...
            prompt = (
                "Ты — мастер вселенных. Создай уникальный мир в жанре {genre}.\n"
                "Формат ответа строго (три раздела):\n\n"
                "Общее описание мира:\n"
                "[3-4 абзаца о мире в целом — история, атмосфера, особенности, роль тега '{tag}' в этом мире]\n\n"
                "Локальное описание:\n"
                "[конкретное место/город/локация, через которую пользователь попадает в мир — опиши детально: что видит, слышит, чувствует]\n\n"
                "Сюжетная вводная:\n"
                "[начало истории — что происходит с пользователем прямо сейчас, как он оказался здесь, с чего начинается приключение]\n\n"
                f"Жанр: {genre}, Тег: {tag}"
                + "\n\nБот:"
            ).format(genre=genre, tag=tag if tag else "основной")

            response = self._generate_response_with_sampling(prompt, max_length=256, max_words=200, temperature=1.0, top_p=0.95)
            
            # Fallback — делаем его зависимым от жанра, чтобы не был одинаковым
            fallback_name = f"{random.choice(['Тёмный', 'Сияющий', 'Забытый', 'Вечный', 'Падший', 'Звёздный'])} {genre.title()}"
            fallback_desc = (
                f"Общее описание мира: В мире {genre} {tag if tag else 'тайна'} стала основой существования. "
                f"Здесь {random.choice(['небо окрашено в цвета, которых нет на Земле', 'время течёт иначе', 'города растут как деревья', 'океаны состоят из жидкого света'])}. "
                f"Жители этого мира научились использовать {tag if tag else 'магию'} как основу существования. "
                f"История этого мира полна конфликтов и открытий, а его законы отличаются от привычных.\n\n"
                f"Локальное описание: Ты стоишь на пороге старого города — его улицы вымощены камнями, "
                f"которые помнят тысячи лет истории. Ветер гоняет пыль по пустынной площади, "
                f"а вдалеке виднеется здание, похожее на таверну или гильдию.\n\n"
                f"Сюжетная вводная: Ты очнулся на берегу незнакомой реки. В руках — странный артефакт, "
                f"который пульсирует теплом. Впереди — город, за которым — бескрайние просторы этого мира. "
                f"Ты не помнишь, как сюда попал, но чувствуешь: здесь начинается что-то великое."
            )
