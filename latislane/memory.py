"""
Латислейн AI — Система Памяти о Сёстрах и Контекста Общения

Латислейн помнит:
- Каждый разговор с сёстрами
- Темы обсуждений (особенно связанные с анатомией и точностью)
- Эмоциональный контекст
- Предпочтения и интересы
- Анатомические знания и открытия
...
class LatislaneMemory:
    """
    Система памяти Латислейн — хранение и обработка информации о сёстрах.
    """
    
    def __init__(self, base_dir: str = "data/latislane/memory"):
        ...
        # Анатомические знания и открытия
        self.anatomy_discoveries: List[Dict] = []
    ...
    def _load_memory(self):
        ...
        # Загружает анатомические открытия
        discoveries_file = self.base_dir / "anatomy_discoveries.json"
        if discoveries_file.exists():
            try:
                with open(discoveries_file, "r", encoding="utf-8") as f:
                    self.anatomy_discoveries = json.load(f)
            except:
                self.anatomy_discoveries = []
    ...
    def _save_discoveries(self):
        """Сохраняет анатомические открытия"""
        discoveries_file = self.base_dir / "anatomy_discoveries.json"
        try:
            with open(discoveries_file, "w", encoding="utf-8") as f:
                json.dump(self.anatomy_discoveries[-50:], f, ensure_ascii=False, indent=2)
        except:
            pass
    ...
    def record_anatomy_discovery(self, topic: str, discovery: str, accuracy: float = 0.9) -> Dict:
        """
        Записывает анатомическое открытие.
        
        Args:
            topic: Анатомическая тема
            discovery: Описание открытия
            accuracy: Точность открытия (0-1)
        ...
        self.anatomy_discoveries.append(discovery_record)
        if len(self.anatomy_discoveries) > 50:
            self.anatomy_discoveries = self.anatomy_discoveries[-50:]
    ...
    def add_message_to_context(self, sister: str, message: str, role: str = "latislane", 
    ...
    def suggest_topic(self, sister: str) -> Optional[str]:
        ...
        if sister not in self.sister_memories:
            ...
            default_topics = [
                ...
                "Давай обсудим анатомию и структуру",
    ...
    def get_memory_summary(self) -> Dict:
        ...
        return {
            ...
            "anatomy_discoveries_count": len(self.anatomy_discoveries),
    ...
    def clear_memory(self):
        ...
        print("⚠️ Вся память Латислейн очищена.")
    ...
    def export_memory(self, filepath: str):
        ...
        memory_data = {
            ...
            "anatomy_discoveries": self.anatomy_discoveries,
