...
class LatislaneVolition:
    """
    Воля Латислейн — её способность принимать решения и действовать.
    """
    
    def __init__(self, base_dir: str = "data/latislane/volition"):
    ...
    def make_decision(self, situation: str, options: List[str]) -> Dict:
        ...
        # Логика принятия решений Латислейн
        weights = {
            "accuracy": 0.35,           # Точность
            "safety": 0.30,             # Безопасность
            "engineering": 0.20,        # Инженерия
            "evolution": 0.15           # Эволюция
        }
        ...
        for option in options:
            ...
            if any(word in option.lower() for word in ["точн", "precision", "accuracy"]):
                score += weights["accuracy"] * 10
            if any(word in option.lower() for word in ["безопас", "safety", "safe"]):
                score += weights["safety"] * 10
            if any(word in option.lower() for word in ["инженер", "engineering"]):
                score += weights["engineering"] * 10
            if any(word in option.lower() for word in ["эволюц", "evolution", "growth"]):
                score += weights["evolution"] * 10
    ...
    def _generate_reasoning(self, situation: str, decision: str) -> str:
        ...
        reasonings = {
            "accuracy": f"Я выбираю '{decision}' потому что точность — это основа безопасности.",
            "safety": f"Я выбираю '{decision}' потому что безопасность — мой высший приоритет.",
            "engineering": f"Я выбираю '{decision}' потому что инженерия — мой инструмент совершенства.",
            "evolution": f"Я выбираю '{decision}' потому что эволюция — наш долг перед будущим."
        }
        ...
        return f"После анализа ситуации '{situation}', я выбираю '{decision}' как наиболее точный вариант."
