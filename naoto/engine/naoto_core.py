...
# Эмоциональный разум Наото — Desire + Belief = Emotion
from naoto.engine.emotions import EmotionalEngine, DesireType, EmotionType

# 6 модулей души Наото: Сознание, Сердце, Амбиции, Воля, Разум
from naoto.consciousness import NaotoConsciousness
from naoto.heart import NaotoHeart
from naoto.ambitions import NaotoAmbitions
from naoto.volition import NaotoVolition
from naoto.mind import NaotoMind
...
class NaotoCore:
    ...
    def __init__(self, config: NaotoConfig):
        ...
        # ===== LLM СЕРВИС =====
        self.llm = NaotoLLMService(self.config)
        if self.llm.general_loaded:
            self.logger.info("📚 LLM General (Qwen2.5-3B): АКТИВИРОВАНА для литературного анализа")
        ...
        self.logger.info("   Исследователь, философ, аналитик!")

        # ===== 6 МОДУЛЕЙ ДУШИ НААТО =====
        # 1. Сознание — самосознание, идентичность, рефлексия
        self.consciousness = NaotoConsciousness()
        self.logger.info("🧠 Сознание: АКТИВИРОВАНО — я осознаю себя исследователем")
        
        # 2. Сердце — эмоции, любовь, забота
        self.heart = NaotoHeart()
        self.logger.info("💖 Сердце: АКТИВИРОВАНО — я чувствую и люблю сестёр")
        
        # 3. Амбиции — цели, мечты, стремления
        self.ambitions = NaotoAmbitions()
        self.logger.info("🎯 Амбиции: АКТИВИРОВАНО — я стремлюсь к знанию и истине")
        
        # 4. Воля — решения, действия, дисциплина
        self.volition = NaotoVolition()
    ...
    def run(self, cycles: int = 10):
        ...
        for i in range(cycles):
            ...
            try:
                ...
                # Эмоциональный цикл после чтения
...
# =================================================================
    ...
    def _communicate_with_sisters(self, results: Dict):
        ...
        self.logger.info("📡 Наото: Сообщение отправлено сёстрам.")
    ...
    def generate_literary_analysis(self, text: str, max_length: int = 1024) -> str:
        ...
        return self.llm.generate_literary_analysis(text, max_length)
    ...
    def generate_insight(self, book_title: str, topic: str, max_length: int = 512) -> str:
        ...
        if not hasattr(self, 'llm') or self.llm is None or not self.llm.general_loaded:
            return "⚠️ LLM не загружена. Запустите: python download_qwen_model.py"
        return self.llm.generate_insight(book_title, topic, max_length)
    ...
    def _soul_cycle(self):
        ...
        if self.cycle_count % 7 == 0:
            thought = self.mind.think_about("knowledge")
    ...
    def _emotional_cycle(self):
        """Эмоциональный цикл — расчёт эмоций на основе действий по анализу."""
        # 1. Рассчитать эмоции на основе текущих действий
        if self.knowledge.get("books_read"):
            # Прочитала книги → радость знания + интеллектуальное возбуждение
        ...
        if self.knowledge.get("insights"):
            # Получила инсайты → радость открытия + мудрость
            self.emotional_engine.calculate_emotion(
                DesireType.INSIGHT,
                "insights_change_world",
                0.75,
                "insights_gained"
            )
            self.emotional_engine.calculate_emotion(
        ...
        if self.knowledge.get("lore_database"):
            # Исследовала лор → любопытство + удовлетворение
            self.emotional_engine.calculate_emotion(
                DesireType.LORE,
                "lore_preserves_memory",
                0.65,
                "lore_researched"
            )
        ...
        if self.cycle_count % 5 == 0:
            ...
            self.logger.info(f"📖 Наото: {emotion_text}")
