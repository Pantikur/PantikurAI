...
CONVERSATIONS_JSON = BASE_DIR / "data" / "conversations.json"  # Только он у нас есть!
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        chatbot = ChatBot(str(MODEL_PATH), str(DATA_PATH))
    ...
    try:
        if req.mode == "narrative":
            ...
            prompt_text = textwrap.dedent(f"""
                ...
                Бот:
            """).strip()
            response = chatbot.generate_response([{"message": prompt_text, "is_own": True}], mode="chat").strip()
            if len(response) < 20:
            ...
            if "Жанр:" in last_msg:
                try: genre = last_msg.split("Жанр:")[1].split(".")[0].strip()
                except: pass
            if "Темы:" in last_msg:
                try: tags = last_msg.split("Темы:")[1].split(".")[0].strip()
                except: pass

            input_msg = f"Создай мир: {genre}"
            if tags: input_msg += f", {tags}"

            response = chatbot.generate_response([{"message": input_msg, "is_own": True}], mode="world_gen").strip()
...
RETRAIN_TOKEN = os.getenv("RETRAIN_TOKEN", "your-birthday-token-2025")  # Меняй в .env!
...
# === WebSocket (с потоковой отправкой) ===
@app.websocket("/ws")
