...
async def predict(request: Request):
    ...
    try:
        ...
        if mode == "narrative":
            ...
            # Формируем промпт для бота — строго 3 части
            prompt = f"Создай мир в жанре {genre}. Дополнительный тег: {tag if tag else 'нет'}. " \
                     "Ответь ТОЧНО в этом формате:\n\n" \
                     "Общее описание мира: [3-4 абзаца о мире в целом — история, атмосфера, особенности, роль тега]\n\n" \
                     "Локальное описание: [конкретное место/город/локация, через которую пользователь попадает в мир — детально]\n\n" \
                     "Сюжетная вводная: [начало истории — что происходит с пользователем прямо сейчас, как он оказался здесь, с чего начинается приключение]\n\n" \
            ...
            # Fallback если ответ слишком короткий — с рандомизацией
            if len(response) < 50:
                sky_variants = [
                    "небо окрашено в цвета, которых нет на Земле",
                    "время течёт иначе — дни сливаются с ночами",
                    "города растут как деревья из земли",
                    "океаны состоят из жидкого света",
                    "луна раскололась на три светящиеся части",
                    "звёзды двигаются по невидимым дорожкам",
                    "небо покрыто вечными северными сияниями",
                    "день и ночь чередуются каждые два часа"
                ]
                ground_variants = [
                    "земля пульсирует тёплым светом",
                    "растения светятся изнутри синим",
                    "каждый камень хранит чьё-то воспоминание",
                    "ветер несёт голоса из далёкого прошлого",
                    "почва переливается всеми цветами радуги",
                    "дороги меняют направление сами по себе",
                    "тени двигаются отдельно от предметов"
                ]
                artifact_variants = [
                    "странный артефакт, пульсирующий тёплым светом",
                    "письмо с незнакомым, но знакомым почерком",
                    "ключ, который не подходит ни к одному замку",
                    "зеркало, показывающее не твоё лицо",
                    "монета с символом, который ты уже видел во сне",
                    "фляга с жидкостью, похожей на воду, но не водой"
                ]
                place_variants = [
                    "незнакомый город, освещённый тысячью огней",
                    "глубокая пещера с кристальными стенами",
                    "заброшенная башня, уходящая в облака",
                    "светящийся лес, полный странных существ",
                    "древний храм, покрытый Runes",
                    "пустынная площадь, где когда-то был рынок"
                ]
                
                response = (
                    f"Название: {random.choice(['Тёмный', 'Сияющий', 'Забытый', 'Вечный', 'Падший', 'Звёздный', 'Багровый', 'Ледяной'])} {genre.title()}\n\n"
                    f"Общее описание мира: В мире {genre} {tag if tag else 'тайна'} стала основой существования. "
                    f"Здесь {random.choice(sky_variants)}. "
                    f"{random.choice(ground_variants)}. "
                    f"Жители научились использовать {tag if tag else 'магию'} как основу существования.\n\n"
                    f"Локальное описание: Ты стоишь на {random.choice(['площади забытого города', 'мосту через бездну', 'тропе между двух гор', 'берегу кристального озера'])}. "
                    ...
                    f"Сюжетная вводная: Ты очнулся здесь с {random.choice(artifact_variants)}. "
                    f"Перед тобой — {random.choice(place_variants)}. "
                    f"Кто-то или что-то ждёт тебя."
                )
                logger.warning("⚠️ Слишком короткий ответ → fallback с рандомизацией")
