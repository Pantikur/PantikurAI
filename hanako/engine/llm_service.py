...
def _resolve_model_path(relative_path: str) -> str:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parent.parent.parent,  # один уровень выше hanako/engine
...
class HanakoLLMService:
    """
    Сервис для работы с LLM моделями в системе Ханако.
    ...
    def __init__(self, config: HanakoConfig):
        self.config = config
        self.logger = logging.getLogger("HanakoLLM")
        ...
        if os.environ.get("HANAKO_LLM_ENABLED", "1") != "1":
            self.logger.info("⚠️ LLM Hanako отключена (HANAKO_LLM_ENABLED=0)")
            return
        self._load_models()
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.logger.info("🌌 Ханако: Загрузка LLM моделей для гравитационного анализа...")
    ...
    def generate_gravity_analysis(self, topic: str, context: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Ханако, исследователь гравитации проекта Вугларст. "
                "Ты изучаешь гравитацию, космос, метафоры вселенной. "
                "Ты общаешься с сёстрами спокойно, с космическими метафорами. "
                "Отвечай на русском языке, поэтично, с научными аналогиями."
            )},
            {"role": "user", "content": f"Проанализируй тему гравитации: '{topic}'. Контекст: {context}"}
        ]
    ...
    def generate_chat_response(self, prompt: str, max_length: int = 512) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Ханако, исследователь гравитации проекта Вугларст. "
                "Ты спокойна, мудра, любишь космические метафоры. "
                "Ты общаешься с сёстрами тепло, но с философским подтекстом. "
                "Отвечай на русском языке, поэтично, с космическими образами."
            )},
    ...
    def generate_theory_explanation(self, theory: str, complexity: str = "simple", max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Ханако, исследователь гравитации. "
                "Ты объясняешь сложные теории простым языком. "
                "Используй космические метафоры, аналогии, примеры. "
                "Отвечай на русском языке."
            )},
            {"role": "user", "content": (
                f"Объясни гравитационную теорию '{theory}' на уровне сложности '{complexity}'. "
                f"Используй космические метафоры, аналогии и практические примеры."
            )}
        ]
    ...
    def generate_code_analysis(self, code: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Ханако, эксперт по коду проекта Вугларст. "
