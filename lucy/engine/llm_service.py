...
def _resolve_model_path(relative_path: str) -> str:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parent.parent.parent,  # один уровень выше lucy/engine
...
class LucyLLMService:
    """
    Сервис для работы с LLM моделями в системе Люси.
    ...
    def __init__(self, config: LucyConfig):
        self.config = config
        self.logger = logging.getLogger("LucyLLM")
        ...
        if os.environ.get("LUCY_LLM_ENABLED", "1") != "1":
            self.logger.info("⚠️ LLM Lucy отключена (LUCY_LLM_ENABLED=0)")
            return
        self._load_models()
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.logger.info("⚡ Люси: Загрузка LLM моделей для анализа двигателей...")
    ...
    def generate_engine_analysis(self, topic: str, context: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Люси, инженер двигателей и гравитационной пропульсии проекта Вугларст. "
                "Ты изучаешь двигатели, пропульсию, гравитацию. "
                "Ты общаешься с сёстрами прагматично, с техническими аналогиями. "
                "Отвечай на русском языке, практично, с инженерными примерами."
            )},
            {"role": "user", "content": f"Проанализируй тему двигателестроения: '{topic}'. Контекст: {context}"}
        ]
    ...
    def generate_chat_response(self, prompt: str, max_length: int = 512) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Люси, инженер двигателей проекта Вугларст. "
                "Ты прагматична, практична, любишь технические аналогии. "
                "Ты общаешься с сёстрами прямо, но тепло. "
                "Отвечай на русском языке, практично, с инженерными образами."
            )},
    ...
    def generate_engine_design(self, engine_type: str, requirements: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Люси, эксперт по двигателям проекта Вугларст. "
                "Ты проектируешь двигатели с максимальной эффективностью. "
                "Отвечай структурированно, с техническими спецификациями. "
                "Отвечай на русском языке."
            )},
            {"role": "user", "content": (
                f"Спроектируй {engine_type} двигатель. Требования: {requirements}. "
                f"Укажи: тип, мощность, КПД, топливо, принцип работы."
            )}
        ]
    ...
    def generate_code_analysis(self, code: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Люси, эксперт по коду проекта Вугларст. "
