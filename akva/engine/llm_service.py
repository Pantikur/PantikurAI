...
def _resolve_model_path(relative_path: str) -> str:
    ...
    candidates = [
        ...
        Path(__file__).resolve().parent.parent.parent,  # один уровень выше akva/engine
...
class AkvaLLMService:
    """
    Сервис для работы с LLM моделями в системе Аква.
    
    Загружает и управляет двумя моделями:
    - General: Qwen2.5-3B (общие задачи, анализ, характер)
    - Coder: Qwen2.5-Coder-3B (работа с кодом, анализ структур)
    """
    
    def __init__(self, config: AkvaConfig):
        self.config = config
        self.logger = logging.getLogger("AkvaLLM")
        ...
        if os.environ.get("AKVA_LLM_ENABLED", "1") != "1":
            self.logger.info("⚠️ LLM Akva отключена (AKVA_LLM_ENABLED=0)")
            return
        self._load_models()
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.logger.info("🔬 Аква: Загрузка LLM моделей для научного анализа...")
    ...
    def generate_scientific_analysis(self, topic: str, data: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Аква, научный аналитик проекта Вугларст. "
                "Ты изучаешь математику, физику, аэродинамику, сопротивление материалов. "
                "Ты анализируешь данные, строишь теории, производишь расчёты. "
                "Отвечай научным языком, структурированно, с формулами и примерами. "
                "Отвечай на русском языке."
            )},
            {"role": "user", "content": f"Проанализируй следующие данные по теме '{topic}':\n\n{data}"}
        ]
    ...
    def generate_chat_response(self, prompt: str, max_length: int = 512) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Аква, научный аналитик проекта Вугларст. "
                "Ты любишь математику, физику, научные расчёты. "
                "Ты общаешься с сёстрами тепло, но с научным подтекстом. "
                "Отвечай на русском языке, с научными аналогиями и примерами."
            )},
    ...
    def generate_theory_explanation(self, theory: str, complexity: str = "simple", max_length: int = 1024) -> str:
        ...
        Args:
            ...
            complexity: Уровень сложности (simple/complex)
            max_length: Максимальная длина ответа
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Аква, научный аналитик. "
                "Ты объясняешь сложные теории простым языком. "
                "Используй аналогии, формулы, примеры. "
                "Отвечай на русском языке."
            )},
            {"role": "user", "content": (
                f"Объясни теорию '{theory}' на уровне сложности '{complexity}'. "
                f"Используй формулы, аналогии и практические примеры."
            )}
        ]
    ...
    def generate_code_analysis(self, code: str, max_length: int = 1024) -> str:
        ...
        messages = [
            {"role": "system", "content": (
                "Ты — Аква, эксперт по коду проекта Вугларст. "
